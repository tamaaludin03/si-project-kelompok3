...
type LampiranItem = { id?: number; nama_file?: string | null; path_file?: string | null };
...
function getLampiran(item: any): LampiranItem[] {
  return item.lampiran || item.lampirans || item.cutiLampiran || item.izinLampiran ||
    item.cuti_lampiran || item.izin_lampiran || item.attachments || item.files || [];
}
...
export default function KaurPengajuanPage() {
  ...
  async function loadData() {
    ...
    const [cutiData, izinData] = await Promise.all([
      safeFetch([`${API_BASE_URL}/cuti/kaur/pending${nipQ}`, `${API_BASE_URL}/cuti/kaur/all`]),
      safeFetch([`${API_BASE_URL}/izin/kaur/pending${nipQ}`, `${API_BASE_URL}/izin/kaur/all`]),
    ]);
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    return [...cutiRows, ...izinRows]
      ...
      .filter((r) => {
        ...
        return !q || [r.nama, r.nip, r.unit, r.jabatan, r.jenis].some((v) => v.toLowerCase().includes(q));
      });
    ...
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }} className="min-h-screen bg-transparent">
      ...
      <div className="mx-auto max-w-6xl space-y-5 px-3 py-4 sm:px-6 sm:py-8">
        ...
        <section className="rounded-3xl border border-slate-100 bg-white p-4 sm:p-7 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-1.5 text-2xl font-extrabold tracking-tight text-slate-900">Daftar Pengajuan</h1>
              <p className="mt-1.5 text-sm text-slate-400">
                Seluruh pengajuan cuti dan izin dari pegawai unit kerja Anda.
              </p>
            </div>
            <div className="flex items-center gap-3">
              <span className="rounded-2xl border border-violet-100 bg-violet-50 px-3.5 py-1.5 text-xs font-bold text-violet-700">
                {cuti.length + izin.length} pengajuan
              </span>
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: "Total",  value: cuti.length + izin.length, color: "violet"  },
        ...
        {loading ? (
          <div className="rounded-3xl border border-slate-100 bg-white p-12 text-center shadow-sm">
            ...
            <p className="mt-3 text-sm text-slate-400">Memuat daftar pengajuan...</p>
          </div>
        ) : rows.length === 0 ? (
          <div className="rounded-3xl border border-slate-100 bg-white p-12 text-center shadow-sm">
            <p className="text-sm font-semibold text-slate-500">Belum ada pengajuan dari unit kerja Anda.</p>
          </div>
        ) : (
          <>
            <p className="px-1 text-xs font-semibold text-slate-400">Menampilkan {rows.length} pengajuan</p>
