...
export default function KaurDataPegawaiPage() {
  const [allData,    setAllData]    = useState<Pegawai[]>([]);
  const [loading,    setLoading]    = useState(true);
  const [error,      setError]      = useState("");
  const [search,     setSearch]     = useState("");
  const [unitKaur,   setUnitKaur]   = useState("");
  ...
  // Filter hanya pegawai di unit KAUR
  const unitData = useMemo(() => {
    if (!unitKaur) return allData;
    return allData.filter((p) =>
      (p.unit || "").toLowerCase().trim() === unitKaur.toLowerCase().trim()
    );
  }, [allData, unitKaur]);
  ...
  const totalL = unitData.filter((p) => p.nama && !p.nama.match(/^$/)).length;
    ...
    <main className="min-h-screen space-y-5 bg-transparent p-3 sm:p-6" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <section className="rounded-3xl border border-slate-100 bg-white p-4 sm:p-7 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-amber-500">Data Pegawai Unit</p>
          ...
          <button
            ...
            disabled={refreshing}
            className="flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2 text-sm font-semibold text-amber-700 transition hover:bg-amber-100 disabled:opacity-60 active:scale-95"
          >
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          { label: "Total Pegawai",     val: unitData.length,                                                                                           b: "border-amber-100",   bg: "bg-amber-50/60",   n: "text-amber-700"   },
          { label: "Cuti Tahunan Aktif",val: unitData.filter(p => (p.kuota?.tahunan?.sisa ?? 12) > 0).length,                                           b: "border-violet-100",  bg: "bg-violet-50/60",  n: "text-violet-700"  },
      ...
      <section className="rounded-3xl border border-slate-100 bg-white p-4 sm:p-7 shadow-sm">
        <div className="mb-5 flex flex-wrap items-center gap-3">
          ...
          <div className="relative min-w-[220px] flex-1">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pl-9 pr-4 text-sm text-slate-800 outline-none transition focus:border-amber-400 focus:bg-white focus:ring-2 focus:ring-amber-400/10"
            />
          </div>
        ...
        {filtered.length === 0 ? (
          <div className="rounded-2xl border border-slate-100 bg-slate-50 py-12 text-center text-sm text-slate-400">
            {unitData.length === 0
              ? `Tidak ada pegawai di unit "${unitKaur}".`
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {filtered.map((p, idx) => {
                  ...
                  return (
                    <tr key={p.id} className="group transition-colors hover:bg-amber-50/30">
