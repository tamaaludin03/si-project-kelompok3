...
export default function DireksiRekapPengajuanPage() {
  const [list, setList]         = useState<Pengajuan[]>([]);
  const [units, setUnits]       = useState<RekapUnit[]>([]);
  ...
  async function load() {
    ...
    try {
      ...
      setUnits(data?.data?.rekapPerUnit   || []);
  ...
  const filtered = useMemo(() => {
    ...
    if (unit !== "Semua") r = r.filter(i => (i.unit || "Tanpa Unit") === unit);
    if (status !== "semua") {
      r = r.filter(i => {
        const s = i.status.toLowerCase();
        if (status === "pending") return s === "pending" || s === "pending_direktur";
        if (status === "proses")  return s.includes("kaur") || s.includes("kabag");
        if (status === "final")   return s.includes("final") || s === "selesai";
        if (status === "ditolak") return s.includes("tolak");
        return true;
      });
    }
    if (search.trim()) {
    ...
    return r;
  }, [list, kategori, unit, status, search]);
  ...
  return (
    <main className="min-h-screen space-y-5 bg-transparent p-3 sm:p-6" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <section className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select
            ...
            className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 outline-none focus:border-violet-400"
          >
          ...
          {/* View toggle */}
          <div className="flex rounded-xl border border-slate-200 bg-slate-50 p-0.5">
            <button onClick={() => setView("kartu")}
              className={`rounded-lg px-3 py-1.5 text-xs font-bold transition ${view === "kartu" ? "bg-white text-violet-700 shadow-sm" : "text-slate-500"}`}
            >Kartu</button>
            <button onClick={() => setView("tabel")}
              className={`rounded-lg px-3 py-1.5 text-xs font-bold transition ${view === "tabel" ? "bg-white text-violet-700 shadow-sm" : "text-slate-500"}`}
            >Tabel</button>
          </div>

          {(search || kategori !== "Semua" || unit !== "Semua" || status !== "semua") && (
            <button
              onClick={() => { setSearch(""); setKategori("Semua"); setUnit("Semua"); setStatus("semua"); }}
          ...
          <span className="ml-auto text-xs text-slate-400">{filtered.length} dari {list.length} pengajuan</span>
        </div>
      </section>
          ...
          <div className="overflow-x-auto rounded-2xl border border-slate-100">
            <table className="min-w-full divide-y divide-slate-100 text-sm">
              <thead className="bg-slate-50">
                <tr>
                  {["No","Kategori","Jenis","Nama","NIP","Unit","Jabatan","Tanggal","Status"].map(h => (
              ...
              <tbody className="divide-y divide-slate-50 bg-white">
                {filtered.map((item, idx) => (
                  <tr key={`${item.kategori}-${item.id}`} className="transition-colors hover:bg-violet-50/30">
                    ...
                    <td className="px-4 py-3 text-slate-500 whitespace-nowrap">{formatDate(item.tanggal)}</td>
                    <td className="px-4 py-3">
                      <span className={`rounded-full border px-2.5 py-0.5 text-[11px] font-bold ${statusBadge(item.status)}`}>
                        {statusLabel(item.status)}
                      </span>
                    </td>
                  </tr>
      ...
      {units.length > 0 && (
        <section className="rounded-3xl border border-slate-100 bg-white p-4 sm:p-7 shadow-sm">
          <div className="mb-5">
            <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-violet-500">Distribusi</p>
            <h2 className="mt-1 text-lg font-extrabold text-slate-900">Rekap Per Unit</h2>
          </div>
          <div className="overflow-x-auto rounded-2xl border border-slate-100">
            <table className="min-w-full divide-y divide-slate-100 text-sm">
              <thead className="bg-slate-50">
