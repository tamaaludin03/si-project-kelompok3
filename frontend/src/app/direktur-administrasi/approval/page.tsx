...
export default function DirekturApprovalPage() {
  ...
  async function handleAction() {
    ...
    const url = `${API_BASE_URL}/${entity}/${modal.id}/${action}-direktur-administrasi`;
  ...
  return (
    ...
    >
      {toast && (
        <div className="fixed top-5 right-5 z-50 rounded-2xl bg-slate-900 px-5 py-3 text-sm font-bold text-white shadow-xl animate-fade-in">
          {toast}
      ...
      <section className="mx-auto max-w-6xl space-y-6">
        <header className="rounded-3xl bg-white border border-slate-100 p-4 sm:p-8 shadow-sm">
          ...
          <h1 className="mt-1.5 text-3xl font-extrabold text-slate-900">Persetujuan Pengajuan Kepala Bagian</h1>
          <p className="mt-1.5 text-sm text-slate-400">Pengajuan cuti dan izin dari Kepala Bagian yang memerlukan persetujuan Anda.</p>
        </header>
                  ...
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div className="flex-1">
                      <div className="flex flex-wrap gap-2 mb-3">
                        ...
                        {item.is_urgent && <span className="rounded-full bg-amber-100 text-amber-700 px-3 py-1 text-xs font-bold">⚡ Mendesak</span>}
                      ...
                      {item.alasan && (
                        <p className="text-sm text-slate-500 mt-1 italic">"{item.alasan}"</p>
                      )}
                    </div>
        ...
        <Modal open onClose={() => { setModal(null); setCatatan(""); setAlasan(""); }} className="max-w-md rounded-3xl">
          <div className="p-8">
            ...
            <div className="flex gap-3 mt-6 justify-end">
              <button
                onClick={() => { setModal(null); setCatatan(""); setAlasan(""); }}
