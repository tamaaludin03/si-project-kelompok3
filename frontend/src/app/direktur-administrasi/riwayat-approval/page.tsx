...
export default function DirekturRiwayatApprovalPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      // Filter hanya yang pernah diproses direktur (sudah ada status_direktur_administrasi)
      setCuti(cutiAll.filter((i: any) => i.status_direktur_administrasi));
      setIzin(izinAll.filter((i: any) => i.status_direktur_administrasi));
      setLoading(false);
    }
    loadData();
  }, []);
  ...
  const rows = useMemo(() => {
    const cutiRows = cuti.map((item) => ({
      ...
      status: item.status || "-",
      catatan: item.catatan_direktur_administrasi || "Tidak ada catatan.",
    ...
    <main className="min-h-screen bg-transparent px-3 py-4 sm:px-6 sm:py-6 text-slate-900" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      <section className="mx-auto max-w-7xl space-y-6">
        <header className="rounded-3xl bg-white border border-slate-100 p-4 sm:p-8 shadow-sm">
          ...
          <h1 className="mt-1.5 text-3xl font-extrabold text-slate-900">Riwayat Approval</h1>
          <p className="mt-1.5 text-sm text-slate-400">Riwayat keputusan approval yang telah Anda proses.</p>
        </header>
