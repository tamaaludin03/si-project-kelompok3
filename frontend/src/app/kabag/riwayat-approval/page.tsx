...
type Kategori = "semua" | "Cuti" | "Izin" | "Urgent";

type Row = {
  id: number;
  kategori: "Cuti" | "Izin";
  isUrgent: boolean;
  nama: string; nip: string; unit: string;
  jenis: string; periode: string; status: string; catatan: string;
  createdAt: string;
};
...
export default function KabagRiwayatApprovalPage() {
  ...
  const allRows = useMemo<Row[]>(() => {
    ...
    return [...cutiRows, ...izinRows].sort((a, b) =>
  ...
  const rows = useMemo(() => {
    return allRows
      .filter((r) => {
        if (kategori === "Cuti")   return r.kategori === "Cuti" && !r.isUrgent;
        if (kategori === "Izin")   return r.kategori === "Izin" && !r.isUrgent;
        if (kategori === "Urgent") return r.isUrgent;
        return true;
      })
      .filter((r) => {
        const q = keyword.toLowerCase();
        return !q || [r.nama, r.nip, r.unit, r.status].some((v) => v.toLowerCase().includes(q));
      });
  }, [allRows, kategori, keyword]);

  const tabs: { key: Kategori; label: string; count: number }[] = [
    { key: "semua",  label: "Semua",  count: allRows.length },
    { key: "Cuti",   label: "Cuti",   count: allRows.filter((r) => r.kategori === "Cuti" && !r.isUrgent).length },
    { key: "Izin",   label: "Izin",   count: allRows.filter((r) => r.kategori === "Izin" && !r.isUrgent).length },
    { key: "Urgent", label: "Mendesak", count: allRows.filter((r) => r.isUrgent).length },
  ];
        ...
        <section className="rounded-3xl border border-slate-100 bg-white p-4 sm:p-7 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-1.5 text-2xl font-extrabold tracking-tight text-slate-900">Riwayat Approval</h1>
              <p className="mt-1.5 text-sm text-slate-400">
                Rekap seluruh persetujuan dan penolakan cuti, izin, dan pengajuan mendesak.
              </p>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex flex-wrap gap-2">
              {tabs.map(({ key, label, count }) => {
                const active = kategori === key;
                const colorMap: Record<Kategori, string> = {
                  semua:  active ? "bg-slate-900 text-white" : "border-slate-200 text-slate-600 hover:bg-slate-50",
                  Cuti:   active ? "bg-violet-700 text-white" : "border-slate-200 text-slate-600 hover:bg-violet-50 hover:text-violet-700",
                  Izin:   active ? "bg-emerald-600 text-white" : "border-slate-200 text-slate-600 hover:bg-emerald-50 hover:text-emerald-700",
                  Urgent: active ? "bg-rose-600 text-white" : "border-slate-200 text-slate-600 hover:bg-rose-50 hover:text-rose-700",
                };
                return (
                  <button key={key} onClick={() => setKategori(key)}
                    className={`flex items-center gap-1.5 rounded-xl border px-4 py-2 text-sm font-semibold transition ${colorMap[key]}`}>
                    {key === "Urgent" && "⚡ "}{label}
                    <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-black ${active ? "bg-white/20 text-white" : "bg-slate-100 text-slate-500"}`}>{count}</span>
                  </button>
                );
              })}
            </div>
        ...
        {loading ? (
          ...
          <div className="rounded-3xl border border-slate-100 bg-white p-12 text-center shadow-sm">
            <p className="text-sm font-semibold text-slate-500">
              {allRows.length === 0 ? "Belum ada riwayat keputusan." : "Tidak ada data yang sesuai dengan filter."}
            </p>
          </div>
        ) : (
          <>
            <p className="px-1 text-xs font-semibold text-slate-400">Menampilkan {rows.length} riwayat · terbaru dahulu</p>
            <div className="space-y-3">
              {rows.map((item) => (
                <div key={`${item.kategori}-${item.id}`}
                  className={`rounded-3xl border bg-white shadow-sm transition hover:shadow-md ${item.isUrgent ? "border-amber-200" : "border-slate-100"}`}>
                  {/* Card header */}
                  <div className={`flex flex-wrap items-start justify-between gap-3 rounded-t-3xl px-6 py-4 ${item.isUrgent ? "bg-amber-50/60" : "bg-slate-50/60"}`}>
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500">NIP {item.nip} &nbsp;·&nbsp; Unit: {item.unit}</p>
                      </div>
                    </div>
                    {/* Type tags — pojok kanan */}
