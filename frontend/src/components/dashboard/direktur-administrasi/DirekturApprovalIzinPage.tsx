...
export default function DirekturApprovalIzinPage() {
  ...
  async function load() {
    setLoading(true);
    const res  = await fetch(`${API_BASE_URL}/izin/direktur/pending`, { headers: authHeaders(), cache: "no-store" });
  ...
  async function handleAction() {
    ...
    setSub(true);
    try {
      const res = await fetch(`${API_BASE_URL}/izin/${modal.id}/${action}`, {
        method: "PATCH",
    ...
    <main className="min-h-screen bg-transparent px-3 py-4 sm:px-6 sm:py-6" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <div className="mx-auto max-w-5xl space-y-5">
        ...
        <section className="rounded-3xl border border-slate-100 bg-white p-4 sm:p-7 shadow-sm">
          ...
          <h1 className="mt-1.5 text-2xl font-extrabold tracking-tight text-slate-900">Approval Izin</h1>
          <p className="mt-1.5 text-sm text-slate-400">Pengajuan izin dari Kepala Bagian yang memerlukan keputusan Anda.</p>
        </section>
        ...
        {loading ? (
          ...
          <div className="rounded-3xl border border-slate-100 bg-white p-12 text-center shadow-sm">
            <p className="text-sm font-semibold text-slate-500">
              {list.length === 0 ? "Tidak ada pengajuan izin yang menunggu persetujuan." : "Tidak ada hasil yang sesuai."}
            </p>
          </div>
        ) : (
          <>
                ...
                <div key={item.id} className={`rounded-3xl border bg-white shadow-sm ${item.is_urgent ? "border-amber-200" : "border-slate-100"}`}>
                  <div className={`flex flex-wrap items-start justify-between gap-3 rounded-t-3xl px-6 py-4 ${item.is_urgent ? "bg-amber-50/60" : "bg-slate-50/60"}`}>
                    <div className="flex items-start gap-3">
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-emerald-100 font-extrabold text-emerald-700">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-[11px] font-bold text-emerald-700">Izin</span>
                  ...
                  <div className="grid grid-cols-1 gap-4 px-6 py-5 sm:grid-cols-3">
                    <div>
                      ...
                      <p className="mt-1 text-sm font-semibold text-slate-800">{(item.jenis_izin || "-").replace(/_/g," ").replace(/\b\w/g,(c:string)=>c.toUpperCase())}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Tanggal / Waktu</p>
                      <p className="mt-1 text-sm font-semibold text-slate-800">
                        {fmt(item.tanggal || item.tanggal_mulai)}
        ...
        <div className="fixed inset-0 z-[600] flex items-center justify-center overflow-y-auto bg-slate-900/40 p-4">
          <div className="w-full max-w-md rounded-3xl bg-white p-8 shadow-2xl">
            ...
            <p className="text-sm text-slate-400 mb-5">
              {modal.action === "approve" ? "Anda akan menyetujui pengajuan izin ini." : "Anda akan menolak pengajuan izin ini. Wajib mengisi alasan."}
            </p>
