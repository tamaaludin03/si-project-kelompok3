...
function getProfile(): Profile {
  ...
  if (raw) {
    try {
      ...
      return {
        nama: user?.nama || user?.name || user?.username || localStorage.getItem("nama") || "KAUR",
        nip: user?.nip || localStorage.getItem("nip") || "-",
        jabatan: user?.jabatan || localStorage.getItem("jabatan") || "KAUR",
  ...
  return {
    nama: localStorage.getItem("nama") || "KAUR",
    nip: localStorage.getItem("nip") || "-",
    jabatan: localStorage.getItem("jabatan") || "KAUR",
...
// ─── StatCard — identical to SdmDashboard ─────────────────────────────────
...
// ─── ActionCard — fixed: no inline object-index, use typed const map ─────────
...
export default function KaurDashboard() {
  ...
  useEffect(() => {
    try {
      setDismissedUrgent(new Set(JSON.parse(localStorage.getItem("dash_dismissed_urgent") || "[]")));
      setDismissedNotif(new Set(JSON.parse(localStorage.getItem("dash_dismissed_notif") || "[]")));
    } catch {}
  }, []);
  ...
  function dismissUrgent(key: string) {
    ...
    try { localStorage.setItem("dash_dismissed_urgent", JSON.stringify([...next])); } catch {}
  }

  function dismissNotif(key: string) {
    const next = new Set([...dismissedNotif, key]);
    setDismissedNotif(next);
    try { localStorage.setItem("dash_dismissed_notif", JSON.stringify([...next])); } catch {}
  }
  ...
  async function loadData() {
    try {
      ...
      const [cutiRes, izinRes, pegawaiRes] = await Promise.all([
        fetch(`${API_BASE_URL}/cuti/kaur/pending${nipQ}`,  { headers, cache: "no-store" }),
        fetch(`${API_BASE_URL}/izin/kaur/pending${nipQ}`,  { headers, cache: "no-store" }),
      ...
      if (cutiRes.ok) setCutiPending(extractItems(cutiJson) as CutiItem[]);
      if (izinRes.ok) setIzinPending(extractItems(izinJson) as IzinItem[]);
  ...
  const rows = useMemo<RowItem[]>(() => {
    const cutiRows: RowItem[] = cutiPending.map((item) => ({
      ...
      alasan:    item.alasan           || "-",
      status:    item.status           || "pending_kaur",
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .kaur-wrap, .kaur-wrap * { font-family: 'Plus Jakarta Sans', sans-serif !important; }
        .kaur-wrap { -webkit-font-smoothing: antialiased; }

        @keyframes kFadeUp {
          from { opacity: 0; transform: translateY(14px); }
          to   { opacity: 1; transform: translateY(0);    }
        }
        .k-a0 { animation: kFadeUp 0.38s cubic-bezier(0.22,1,0.36,1) both; }
        .k-a1 { animation: kFadeUp 0.38s 0.07s cubic-bezier(0.22,1,0.36,1) both; }
        .k-a2 { animation: kFadeUp 0.38s 0.14s cubic-bezier(0.22,1,0.36,1) both; }
        .k-a3 { animation: kFadeUp 0.38s 0.21s cubic-bezier(0.22,1,0.36,1) both; }

        .k-row:hover { background: rgba(245,243,255,0.55); }
      `}</style>
      ...
      <main className="kaur-wrap min-h-screen bg-transparent p-3 sm:p-6 text-slate-900">
        <div className="mx-auto max-w-7xl space-y-5">
          ...
          <section className="k-a0 rounded-3xl border border-slate-100 bg-white p-4 sm:p-7 shadow-sm">
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex shrink-0 flex-col items-end gap-3">
                <div className="flex items-center gap-2">
                  {/* Bell notifikasi */}
                  <a href="/kaur/notifikasi" className="relative flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 transition hover:border-violet-300 hover:text-violet-600">
            ...
            <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4">
              <StatCard
                ...
                color="violet"
                onClick={() => router.push("/kaur/approval-cuti")}
                icon={
              ...
              <StatCard
                ...
                color="emerald"
                onClick={() => router.push("/kaur/approval-izin")}
                icon={
                  <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
          ...
          {/* ── Warning banner ── */}
          {message && (
            <div className="k-a1 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-3.5 text-sm font-semibold text-amber-800 shadow-sm">
              {message}
            </div>
          )}

          {/* ════ ACTION CARDS ════ */}
          <section className="k-a1 rounded-3xl border border-slate-100 bg-white p-4 sm:p-7 shadow-sm">
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
              <ActionCard title="Persetujuan Cuti"  desc="Setujui atau tolak pengajuan cuti"          href="/kaur/approval-cuti"    color="violet"  />
              <ActionCard title="Persetujuan Izin"  desc="Setujui atau tolak pengajuan izin"          href="/kaur/approval-izin"    color="emerald" />
              <ActionCard title="Data Pegawai Unit" desc="Lihat pegawai pada unit kerja"              href="/kaur/data-pegawai"     color="amber"   />
              <ActionCard title="Riwayat Keputusan" desc="Riwayat persetujuan yang pernah diproses"  href="/kaur/riwayat-approval" color="violet"  />
            </div>
          </section>

          {/* ════ MAIN GRID ════ */}
          <div className="k-a2 grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,2fr)_minmax(300px,1fr)]">

            {/* ── Approval table ── */}
            <section className="rounded-3xl border border-slate-100 bg-white p-4 sm:p-7 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <h2 className="text-lg font-extrabold text-slate-900">Pengajuan Menunggu Persetujuan</h2>
                  <p className="mt-1 text-sm text-slate-400">
                    Pengajuan terbaru dari pegawai unit Anda yang perlu diproses.
                  </p>
                </div>
                <button
                  type="button"
              ...
              <div className="mt-4 max-h-64 overflow-y-auto overflow-x-auto rounded-2xl border border-slate-100">
                <table className="min-w-full divide-y divide-slate-100 text-sm">
                  <thead className="bg-slate-50">
                    <tr>
                      {["Pegawai", "Pengajuan", "Periode", "Status", "Aksi"].map((h) => (
                        <th
                          key={h}
                  ...
                  <tbody className="divide-y divide-slate-50 bg-white">
                    {loading ? (
                      ...
                      <tr>
                        <td colSpan={5} className="px-4 py-10 text-center text-sm text-slate-400">
                          Tidak ada pengajuan pending. Semua sudah diproses.
                        </td>
                      </tr>
                    ) : (
                      recentRows.map((item) => (
                        <tr
                          ...
                          className="k-row transition-colors"
                        >
            ...
            <div className="space-y-5">
              ...
              <section className="rounded-3xl border border-slate-100 bg-white p-4 sm:p-7 shadow-sm">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-slate-400">Prioritas penanganan segera.</p>
                  </div>
                  ...
                  {urgentRows.filter(i => !dismissedUrgent.has(`${i.kategori}-${i.id}`)).length === 0 ? (
                    <div className="rounded-2xl border border-dashed border-slate-200 p-8 text-center">
                      ...
                      <p className="mt-1 text-xs text-slate-400">
                        Pengajuan urgent akan muncul otomatis di sini.
                      </p>
                    </div>
                  ) : (
                    <div className="max-h-80 overflow-y-auto space-y-2.5">
                      {urgentRows.filter(i => !dismissedUrgent.has(`${i.kategori}-${i.id}`)).map((item) => (
                        <div key={`urgent-${item.kategori}-${item.id}`}
                          ...
                          <button
                            type="button"
                            onClick={() => router.push(item.kategori === "Cuti" ? "/kaur/approval-cuti" : "/kaur/approval-izin")}
                            className="min-w-0 flex-1 text-left"
                          >
                            <p className="font-semibold text-slate-800">{item.nama}</p>
                            <p className="mt-0.5 text-xs text-slate-500">{item.kategori} • {item.jenis}</p>
                          </button>
              ...
              <section className="rounded-3xl border border-slate-100 bg-white p-4 sm:p-7 shadow-sm">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-bold text-slate-800">Notifikasi Realtime</p>
                    <p className="mt-0.5 text-xs text-slate-400">Pengajuan terbaru masuk ke KAUR.</p>
                  </div>
                          ...
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              <p className="text-sm font-semibold text-slate-800">
                                Pengajuan {item.kategori.toLowerCase()} baru
                              </p>
                              <p className="mt-0.5 text-xs text-slate-500">{item.nama}</p>
                              <p className="mt-0.5 text-xs text-slate-400">{item.jenis} • {item.periode}</p>
                            </div>
                            {item.isUrgent && (
